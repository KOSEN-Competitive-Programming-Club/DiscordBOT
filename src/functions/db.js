...
exports.update = async function run(dbName, collectionName, filter, update) {
  try {
    ...
    await system.error(
      `${dbName}.${collectionName}を更新できませんでした`,
      err,
      `DB更新失敗`,
    );
  }
...
exports.insert = async function run(dbName, collectionName, object) {
  try {
    ...
    await system.error(
      `${dbName}.${collectionName}にレコードを追加できませんでした`,
      err,
      `DB追加失敗`,
    );
  }
 ...
 * @returns {Promise<void>}
 */
exports.updateOrInsert = async function run(
  dbName,
  collectionName,
  filter,
  object,
...
exports.delete = async function run(dbName, collectionName, filter) {
  try {
    ...
    await system.log(
      `${dbName}.${collectionName}からレコード削除(削除されたとは言っていない)`,
      `DBレコード削除操作実行`,
    );
  } catch (err) {
    await system.error(
      `${dbName}.${collectionName}からレコードを削除できませんでした`,
      err,
      `DB削除失敗`,
    );
  }
